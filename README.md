...
Production-сайт-портфолио с тёмной liquid-glass эстетикой: анимированные blob-фоны,
backdrop-blur стекло, ripple-эффект на кликабельных элементах.
...
- **Главная** — блок «обо мне» (текст и ссылки редактируются в админке), последние 5 релизов
  по всем проектам, виджет Spotify (текущий трек; если ничего не играет — последний прослушанный).
...
- **Публикации** — markdown-посты из админки и импортированные из Telegram-канала,
  бейдж источника, сортировка по дате.
...
- **Админка** — вкладки «Проекты GitHub» (показать/скрыть, Hard/Vibe, «Синхронизировать сейчас»),
  «Обо мне и ссылки», «Публикации» (markdown-редактор с превью, публикация/черновик/удаление,
  «Парсить посты из Telegram»).
...
| `TELEGRAM_BOT_TOKEN` | Бот для импорта постов канала (бот — админ канала) |
| `TELEGRAM_CHANNEL` | Опциональный фильтр: `@username` канала |
